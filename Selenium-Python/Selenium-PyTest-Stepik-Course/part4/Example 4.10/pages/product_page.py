...
class ProductPage(BasePage):
    ...
    def should_be_basket_link(self):
            assert self.is_element_present(\
                *ProductPageLocators.BASKET_LINK), "Basket Form is not presented"
    
    def should_not_be_product_in_basket(self):
        assert self.is_not_element_present(\
                *ProductPageLocators.PRODUCT_IN_BASKET), "basket is empty"
...
# Проверка элемента - не появляется
    def should_not_be_success_message_in_basket(self):
        assert self.is_not_element_present(\
            *ProductPageLocators.PRODUCT_TEXT_IN_BASKET),\
                "Success message is presented, but should not be"
